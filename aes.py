from pure import s_box, rcon, inv_s_box
import numpy as np
import random
...
class AES:
    ...
    def generate_key(self, bits: int) -> None:
        ...
        if bits % 8 != 0:
            bytes_length += 1
        self.hex_key: str = ''.join(random.choice('0123456789abcdef') for _ in range(bytes_length * 2))
        #self.hex_key = "2b7e151628aed2a6abf7158809cf4f3c"
        byte_array = np.array([int(self.hex_key[i:i+2], 16) for i in range(0, len(self.hex_key), 2)])
        matrix = np.array([byte_array[i:i+4] for i in range(0, len(byte_array), 4)])
        self.key = matrix.T
    
    def key_expansion(self) -> None:
        self.expanded_key: np.ndarray = np.zeros((4 * (self.rounds + 1), 4), dtype=np.uint8)
    ...
    def mix_columns(self) -> None:
        for i in range(4):
            col: np.ndarray = self.state[:, i]
            self.state[:, i] = [
                self.galois_mult(col[0], 2) ^ self.galois_mult(col[1], 3) ^ col[2] ^ col[3],
                col[0] ^ self.galois_mult(col[1], 2) ^ self.galois_mult(col[2], 3) ^ col[3],
                col[0] ^ col[1] ^ self.galois_mult(col[2], 2) ^ self.galois_mult(col[3], 3),
                self.galois_mult(col[0], 3) ^ col[1] ^ col[2] ^ self.galois_mult(col[3], 2)
            ]
    ...
    def inv_mix_columns(self) -> None:
        for i in range(4):
            col = self.state[:, i]
            self.state[:, i] = [
                self.galois_mult(col[0], 14) ^ self.galois_mult(col[1], 11) ^ self.galois_mult(col[2], 13) ^ self.galois_mult(col[3], 9),
                self.galois_mult(col[0], 9) ^ self.galois_mult(col[1], 14) ^ self.galois_mult(col[2], 11) ^ self.galois_mult(col[3], 13),
                self.galois_mult(col[0], 13) ^ self.galois_mult(col[1], 9) ^ self.galois_mult(col[2], 14) ^ self.galois_mult(col[3], 11),
                self.galois_mult(col[0], 11) ^ self.galois_mult(col[1], 13) ^ self.galois_mult(col[2], 9) ^ self.galois_mult(col[3], 14)
            ]
